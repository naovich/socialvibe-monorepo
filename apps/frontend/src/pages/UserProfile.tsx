          ...
          <div className="px-8 pb-8 flex flex-col items-center md:items-start">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between w-full gap-6">
              <div className="text-center md:text-left">
                <h1 className="text-3xl md:text-4xl font-black text-text-primary tracking-tight">{user.name}</h1>
                <p className="text-text-muted font-medium mt-1">@{user.username}</p>
                <p className="text-text-muted font-medium mt-1">{user.friendsCount} Friends • {user.postsCount} Posts</p>
              </div>

              <div className="flex gap-2">
                {isFriend ? (
                  <button className="flex items-center gap-2 px-6 py-2.5 bg-white/10 text-text-primary rounded-xl font-bold transition-all">
                    <UserCheck size={18} />
                    Friends
                  </button>
                ) : requestSent ? (
                  <button className="flex items-center gap-2 px-6 py-2.5 bg-white/10 text-text-muted rounded-xl font-bold cursor-not-allowed">
                    Request Sent
                  </button>
                ) : (
                  <button
            ...
            <div className="bg-[#1a1a1a] border border-white/10 rounded-3xl p-6 shadow-xl">
              ...
              {user.bio && (
                <div className="text-center mb-6 px-4 italic">
                  "<SafeHTML content={user.bio} className="text-text-disabled text-sm leading-relaxed inline" as="span" />"
                </div>
              )}
            </div>

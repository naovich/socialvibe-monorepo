    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="relative bg-[#1a1a1a] rounded-b-3xl overflow-hidden shadow-2xl border-x border-b border-white/5">
        <div className="h-64 md:h-80 relative group">
          ...
          <button className="absolute bottom-4 right-4 flex items-center gap-2 px-4 py-2 bg-black/60 hover:bg-black/80 backdrop-blur-md rounded-xl text-sm font-bold text-text-primary transition-all border border-white/10 shadow-xl z-10">
            <Camera size={18} />
        ...
        <div className="px-8 pb-8 flex flex-col items-center md:items-start">
          <div className="relative -mt-24 md:-mt-32 mb-4">
            ...
            <button className="absolute bottom-4 right-4 p-2.5 bg-white/10 hover:bg-white/20 backdrop-blur-md rounded-full text-text-primary transition-all border border-white/10 z-10">
              <Camera size={20} />
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between w-full gap-6">
            <div className="text-center md:text-left">
              <h1 className="text-3xl md:text-4xl font-black text-text-primary tracking-tight">{currentUser.name}</h1>
              <p className="text-text-muted font-medium mt-1">{currentUser.friendsCount} Friends • {currentUser.postsCount} Posts</p>
            ...
            <div className="flex gap-2">
              <button 
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-orange-600 hover:bg-orange-500 text-text-primary rounded-xl font-bold transition-all shadow-lg shadow-orange-600/20"
              >
                <Edit3 size={18} />
                Edit Profile
              </button>
          ...
          <div className="flex items-center gap-2 w-full overflow-x-auto scrollbar-hide">
            {['Posts', 'About', 'Friends', 'Photos', 'Videos', 'Check-ins'].map((tab, i) => (
              <button 
                key={tab}
                className={`px-6 py-3 rounded-xl text-sm font-bold transition-all whitespace-nowrap ${i === 0 ? 'bg-white/10 text-orange-500' : 'text-text-muted hover:bg-white/5 hover:text-text-primary'}`}
              >
                {tab}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        ...
        <div className="md:col-span-2 flex flex-col gap-6">
          <div className="bg-[#1a1a1a] border border-white/10 rounded-3xl p-6 shadow-xl">
            <h2 className="text-xl font-black text-text-primary mb-4">Intro</h2>
            {currentUser.bio && (
              <p className="text-text-disabled text-sm leading-relaxed text-center mb-6 px-4 italic">
                "{currentUser.bio}"
              </p>
            )}
            <button className="w-full py-2 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold text-text-disabled transition-all mb-6">
              Edit Bio
            </button>
            <div className="flex flex-col gap-4">
              <div className="flex items-center gap-3 text-sm text-text-muted">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                <span>Fullstack Developer at <span className="text-text-primary font-bold">SocialVibe</span></span>
              </div>
              <div className="flex items-center gap-3 text-sm text-text-muted">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                <span>Lives in <span className="text-text-primary font-bold">Casablanca, Morocco</span></span>
              </div>
              <div className="flex items-center gap-3 text-sm text-text-muted">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Joined <span className="text-text-primary font-bold">May 2024</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#1a1a1a] border border-white/10 rounded-3xl p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-black text-text-primary">Photos</h2>
